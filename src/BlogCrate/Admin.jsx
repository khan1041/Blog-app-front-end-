



import React, { useEffect, useState } from 'react'
import { ImSortAmountAsc } from 'react-icons/im'

function Admin() {

  const[data,setdata]=useState([]) 

const admindata=async()=>{

    try {
        const response=await fetch('https://blog-app-backend-project-1.onrender.com/app/auth/admin',{
            method:"GET",
        })
    
        if(response.ok){
            const listadmin=await response.json()
            setdata(listadmin.adminget)

        }
      
    } catch (error) {
        console.log(error)
    }
   
}
console.log(data)
useEffect(()=>{
admindata()
},[])

  return (
    <div>
      Admin

      <div className="flex flex-wrap justify-center  items-center my-20 bg-gray-100">
      {data.map((creator) => (
        <div
          key={creator._id}
          className=" shadow-lg rounded-lg overflow-hidden max-w-xs w-full m-2 bg-white"
        >
          <div className="relative">
            <img
              src={creator.photo}
              alt="avatar"
              className="w-full h-32 object-cover"
            />
            <div className="absolute inset-x-0 bottom-0 transform translate-y-1/2">
              <img
                src={creator.photo}
                alt="avatar"
                className="w-16 h-16 rounded-full mx-auto border-4 border-gray-700"
              />
            </div>
          </div>
          <div className="px-4 py-6 mt-4">
            <h2 className="text-center text-xl font-semibold text-gray-800">
              {creator.name}
            </h2>
            <p className="text-center text-gray-600 mt-2">{creator.email}</p>
            <p className="text-center text-gray-600 mt-2">{creator.phone}</p>
            <p className="text-center text-gray-600 mt-2">{creator.role}</p>
          </div>
        </div>
      ))}
    </div>
    </div>
  )
}

export default Admin
















